...
"""Script Description:
    ...
    This script is modified version of BETA-Binding and Expression Targets Analysis (BETA),
    which is suitable for analyzing datastes from Non-model species sequencing data.
    It integrates ChIP-seq/CUT&Tag binding data and RNA-seq differentail expression data to predict a factor's targets.

    Part1:
    use binding data to calculate the regulatory potential score of each gene to be regulated by factor.
    1. For each gene in genome, input a parameter ‘distance(d)’. All binding sites near the transcription start site of the gene within the specified range (10 kb as default) are considered.
...
def prepare_optparser():
        usage = """usage: %prog <-p binding_file> <-n name> <-g genome> <-d distance> [options]
            example : python BETA_intergration.py -p TF1.narrowpeak -n TF1 -g /public/home/shipy3/DB/dm6/annotation/BETA_input.txt"""
        description = "BETA for species --- Binding Expression Target Analysis for species"
        ...
        optparser.add_option("-g","--genome",dest="genome",type="string",
                                         help="Select an annottion file with the UCSC bed format file to search genes.")
...
class PScore:
    ...
    def ScoreCalc(self, distance):
        ...
        for igene in self.geneInfo:
            ...
            peaksInDistance = [abs((t[1]+t[2])/2-gTSS)*1.0/distance for t in peaks if abs((t[1]+t[2])/2-gTSS) < distance ]
        ...
        for igene in self.geneInfo:
            ...
            for t in peaks:
                if abs((t[1]+t[2])/2-gTSS) < distance:
                    peak2gene_distance = (t[1]+t[2])/2-gTSS
...
if __name__ == "__main__":
    main()
    # python BETA_intergration.py -p TF1.narrowpeak -n TF1 -g /public/home/shipy3/DB/dm6/annotation/BETA_input.txt
